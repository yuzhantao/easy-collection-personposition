 ...
 *
 */
public class Tag3Decoder implements ITagDecoder<Tag3Vo> {
	private final static Logger log = LogManager.getLogger(Tag0Decoder.class);
	
	@Override
	public List<Tag3Vo> decoder(PersonPositionMessage msg) {
		List<Tag3Vo> tags = new ArrayList<>();
		// Data字段标签个数
		byte tagCount = msg.getData()[0];
		log.info("该Data段的标签3个数为:{}",tagCount);
		// Data字段标签,含有多个相同标签,i表示字节
		for(int i = 1; i < msg.getData().length; i += 3) {
			// 标签3类型,3字节全部表示tagId,不含任何特征位
			Tag3Vo tag3 = new Tag3Vo();
